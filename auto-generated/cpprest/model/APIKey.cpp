...
void APIKey::fromJson(web::json::value& val)
{
    ...
    }
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("permissions")].as_array() )
        {
            if(item.is_null())
            {
                m_Permissions.push_back( std::shared_ptr<any>(nullptr) );
            }
            else
            {
                std::shared_ptr<any> newItem(new any());
...
std::vector<std::shared_ptr<any>>& APIKey::getPermissions()
{
    return m_Permissions;
}

void APIKey::setPermissions(std::vector<std::shared_ptr<any>> value)
{
    m_Permissions = value;
